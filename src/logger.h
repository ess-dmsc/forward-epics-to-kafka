...
#define LOG(level, fmt, ...) { dwlog(level, fmt, __FILE__, __LINE__, __PRETTY_FUNCTION__, __VA_ARGS__); }

#define QLOG(level, fmt, ...) { dwlog(level, "[%lx] " fmt, __FILE__, __LINE__, __PRETTY_FUNCTION__, (uint64_t)QThread::currentThreadId(), __VA_ARGS__); }
