...
#if DO_FLOG
using fmt::print;
#define FLOG(level, fmt, args...)  if (level < 60) { print("{:{}s}" fmt "\n", "", 2*(level), ## args); }
#else
...
typedef struct { Value type; flatbuffers::Offset<void> off; } V_t;

V_t Field(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVField const * field, int level);

inline static V_t field_PVStructure(flatbuffers::FlatBufferBuilder & builder, PVStructure const * field, int level) {
...
inline static V_t field_PVStructure_array(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVValueArray<epics::pvData::PVStructurePtr> const * field, int level) {
...
inline static V_t field_PVScalar(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVScalar const * field, int level) {
	FLOG(level, "scalar");
	auto stype = field->getScalar()->getScalarType();
	#define M(T, B, E, VT) if (stype == epics::pvData::ScalarType::E) { \
		auto p1 = reinterpret_cast<epics::pvData::PVScalarValue<T> const *>(field); \
		B b(builder); \
		b.add_value(p1->get()); \
	...
	if (stype == epics::pvData::ScalarType::pvString) {
		auto p1 = reinterpret_cast<epics::pvData::PVScalarValue<std::string> const *>(field);
	...
	if (stype == epics::pvData::ScalarType::pvBoolean) {
		...
		auto p1 = reinterpret_cast<epics::pvData::PVScalarValue<bool> const *>(field);
...
inline static V_t field_PVScalar_array(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVScalarArray const * field, int level) {
	...
	if (stype == epics::pvData::ScalarType::TE) { \
		auto p1 = reinterpret_cast<epics::pvData::PVValueArray<TC> const *>(field); \
		auto view = p1->view(); \
		TC * a1 = nullptr; \
		auto v1 = builder.CreateUninitializedVector(view.size(), sizeof(TC), (uint8_t**)&a1); \
	...
	if (stype == epics::pvData::ScalarType::pvString) {
		auto p1 = reinterpret_cast<epics::pvData::PVValueArray<std::string> const *>(field);
...
inline static V_t field_PVUnion(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVUnion const * field, int level) {
	FLOG(level, "union");
	auto f3 = field->get();
	if (f3) {
...
V_t Field(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVField const * field, int level) {
	...
	if (etype == epics::pvData::Type::structure) {
		return field_PVStructure(builder,
			reinterpret_cast<epics::pvData::PVStructure const *>(field),
			level+1
		);
	}
	else if (etype == epics::pvData::Type::structureArray) {
		// Serialize all objects, collect the offsets, and store an array of those.
		return field_PVStructure_array(builder,
			reinterpret_cast<epics::pvData::PVValueArray<epics::pvData::PVStructurePtr> const *>(field),
			level+1
		);
	}
	else if (etype == epics::pvData::Type::scalar) {
		return field_PVScalar(builder,
			reinterpret_cast<epics::pvData::PVScalar const *>(field),
			level+1
		);
	}

	else if (etype == epics::pvData::Type::scalarArray) {
		return field_PVScalar_array(builder,
			reinterpret_cast<epics::pvData::PVScalarArray const *>(field),
			level+1
		);
	}

	else if (etype == epics::pvData::Type::union_) {
		return field_PVUnion(builder,
			reinterpret_cast<epics::pvData::PVUnion const *>(field),
			level+1
		);
	}
...
V_t Field(flatbuffers::FlatBufferBuilder & builder, epics::pvData::PVFieldPtr const & field, int level) {
...
void config(std::map<std::string, int64_t> const & config_ints, std::map<std::string, std::string> const & config_strings) override {
...
FlatBufs::SchemaRegistry::Registrar<Info> g_registrar_info("f143", Info::ptr(new Info));
