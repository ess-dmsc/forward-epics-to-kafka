...
TEST_F(ConvertTDCTest, TestFBContents) {
  ...
  auto Result = TestConverter.create(Update);
  EXPECT_NE(Result, nullptr);
  auto FBResult = GetSampleEnvironmentData(Result->builder->GetBufferPointer());
  EXPECT_EQ(FBResult->Name()->str(), Update.channel);
  EXPECT_EQ(FBResult->Values()->size(), 2);
  EXPECT_EQ(FBResult->Timestamps()->size(), 2);
  for (size_t i = 0; i < FBResult->Values()->size(); i++) {
    EXPECT_EQ(FBResult->Values()->Get(i), 0);
  }
