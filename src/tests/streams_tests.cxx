...
std::shared_ptr<MockStream> createMockStream(std::string provider_type, std::string channel_name) {
...
TEST(StreamsTest, back_returns_correct_stream_when_streams_are_added) {
  ...
  streams.add(x);
  ASSERT_EQ(x.get()->channel_info().channel_name, streams.back()->channel_info().channel_name);
}

TEST(StreamsTest, back_returns_nullptr_when_streams_is_empty_after_initialisation) {
  Streams streams;
  ASSERT_EQ(streams.back(), nullptr);
}

TEST(StreamsTest, back_throws_range_error_when_streams_is_empty_after_clear_streams_is_called) {
  Streams streams;
  streams.add(createMockStream("hello", "world"));
  streams.add(createMockStream("world", "hello"));
  streams.streams_clear();
...
TEST(StreamsTest, check_stream_status_on_one_stream_with_negative_status_removes_stream) {
  Streams streams;
...
TEST(StreamsTest, check_stream_status_with_multiple_streams_with_negative_statuses_removes_all_streams) {
  Streams streams;
...
TEST(StreamsTest, check_stream_status_with_multiple_streams_with_negative_statuses_removes_all_streams_and_leaves_positive_status_streams) {
  Streams streams;
