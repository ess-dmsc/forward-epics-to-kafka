...
class NTScalarArrayTestSetUp : public ::testing::Test {
public:
...
TEST_F(NTScalarArrayTestSetUp, NTScalarArrayToFBSuccess) {
    auto testData = CreateTestNTScalarArray<pv::PVIntArray>(100);
    EXPECT_TRUE(ExtractNTScalarArrayData(&fb_builder, testData));
}
