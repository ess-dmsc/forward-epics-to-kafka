...
public:
    ...
    MOCK_METHOD4(metadata, RdKafka::ErrorCode(bool, const RdKafka::Topic*, RdKafka::Metadata**, int));
...
TEST_F(ProducerTests, callPollTest) {
    ...
    EXPECT_CALL(*dynamic_cast<MockHandle*>(Producer1.ProducerPtr.get()), poll(::testing::_)).Times(AtLeast(1));
    Producer1.poll();
}
