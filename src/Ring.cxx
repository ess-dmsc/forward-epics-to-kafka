...
template <typename TP>
Ring<TP>::Ring(uint32_t n) {
	ulock(mx);
	resize_unsafe(n);
}

template <typename TP>
int Ring<TP>::resize(uint32_t n) {
	ulock(mx);
	return resize_unsafe(n);
}

template <typename TP>
int Ring<TP>::resize_unsafe(uint32_t n) {
	n = std::min(n, cap_max);
...
template <typename TP>
int Ring<TP>::push(TP & x) {
	ulock(mx);
	return push_unsafe(x);
}

template <typename TP>
int Ring<TP>::push_unsafe(TP & x) {
	if (iW >= iR) {
...
template <typename TP>
int Ring<TP>::push_enlarge(TP & x) {
	ulock(mx);
	return push_enlarge_unsafe(x);
}

template <typename TP>
int Ring<TP>::push_enlarge_unsafe(TP & p) {
...
template <typename TP>
std::pair<int, TP> Ring<TP>::pop() {
	ulock(mx);
	return pop_unsafe();
}

template <typename TP>
std::pair<int, TP> Ring<TP>::pop_unsafe() {
	if (iR < iW) {
		auto & v = vec[iR];
		inc_R();
		return {0, std::move(v)};
...
template <typename TP>
uint32_t Ring<TP>::size() {
	ulock(mx);
	return size_unsafe();
}

template <typename TP>
uint32_t Ring<TP>::size_unsafe() {
	if (iW >= iR) {
		return iW - iR;
	}
	else {
...
template <typename TP>
uint32_t Ring<TP>::capacity() {
	ulock(mx);
	return capacity_unsafe();
}

template <typename TP>
uint32_t Ring<TP>::capacity_unsafe() {
	auto n = vec.size();
	if (n == 0) return 0;
	return n - 1;
}
...
template <typename TP>
std::vector<char> Ring<TP>::to_vec() {
	ulock(mx);
	return to_vec_unsafe();
}

template <typename TP>
std::vector<char> Ring<TP>::to_vec_unsafe() {
	std::vector<char> ret;
	uint32_t i1 = 0;
	...
	for (auto & e : vec) {
		...
		if (i1 == iRmax-1) {
			...
			if (i1 == iW && i1 == iR) { n1 = 3; n2 = 1; }
			else if (i1 == iR) { n1 = 2; n2 = 1; }
			else if (i1 == iW) { n1 = 1; n2 = 1; }
		}
		uint16_t p1 = 0;
		if (e) {
			if (formatter) p1 = formatter(e);
			else p1 = 0xffff;
		}
		auto s1 = fmt::format("{}{:04x}{} ",
			col1[n1],
			p1,
			col2[n2]
		);
...
template <typename TP>
typename Ring<TP>::ulock Ring<TP>::lock() {
	return ulock(mx);
}
