...
ProducerTopic::ProducerTopic(std::shared_ptr<Producer> ProducerPtr,
  ...
  LOG(spdlog::level::trace, "ctor topic: {}", RdKafkaTopic->name());
}
...
int ProducerTopic::produce(std::unique_ptr<ProducerMessage> &Msg) {
  ...
  case RdKafka::ERR__QUEUE_FULL:
    ...
    LOG(spdlog::level::warn, "Producer queue full, outq: {}",
  ...
  default:
    ...
    LOG(spdlog::level::err, "Publishing message on topic \"{}\" failed",
        RdKafkaTopic->name());
    break;
  }
  return 1;
