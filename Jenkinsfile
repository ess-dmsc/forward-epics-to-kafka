...
node('docker && eee') {
    ...
    try {
        ...
        stage('Check Coverage') {
            sh "/usr/bin/pip install --target=./coveralls cpp-coveralls && ls ./coveralls/cpp_coveralls/ && ./coveralls/cpp_coveralls/coveralls -t 'xtf16Nv5y5SdMjUtFQpuBLaYpizESdGRU' -e src/tests/"
        }
