...
def test_forwarding_configured_from_file(docker_compose):
    ...
    check_expected_value(first_msg, Value.Double, PVDOUBLE_WITH_ALARM_THRESHOLDS, initial_value)
    check_expected_alarm_status(first_msg, AlarmStatus.UDF, AlarmSeverity.NO_ALARM)
    assert(msg_key == PVDOUBLE_WITH_ALARM_THRESHOLDS.encode('utf-8')), 'Message key expected to be the same as the PV name'
    ...
    check_expected_value(second_msg, Value.Double, PVDOUBLE_WITH_ALARM_THRESHOLDS, first_updated_value)
    ...
    check_expected_value(third_msg, Value.Double, PVDOUBLE_WITH_ALARM_THRESHOLDS, second_updated_value)
    check_expected_alarm_status(third_msg, AlarmStatus.NO_CHANGE, AlarmSeverity.NO_CHANGE)
