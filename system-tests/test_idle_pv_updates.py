...
def test_forwarder_sends_idle_pv_updates(docker_compose_idle_updates):
    ...
    for i in range(3):
        ...
        assert msg.Status() != AlarmStatus.NO_CHANGE, "Expect logs from periodic updates to always contain the current EPICS alarm status"
        assert msg.Severity() != AlarmSeverity.NO_CHANGE, "Expect logs from periodic updates to always contain the current EPICS alarm severity"
    consumer.close()
